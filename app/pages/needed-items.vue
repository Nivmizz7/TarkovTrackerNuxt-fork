...
<script setup lang="ts">
  ...
  const filteredItems = computed(() => {
    ...
    // Filter out owned items (fully collected)
    if (hideOwned.value) {
    ...
    if (search.value) {
      items = items.filter((item) => {
        ...
        // Check item name and short name
        ...
        if (item.needType === 'hideoutModule') {
          ...
          if (station?.name) {
            // Match station name alone
            if (fuzzyMatch(station.name, search.value)) {
              return true;
            }
            // Match station name with level
    ...
    return items.sort((a, b) => {
      ...
      switch (sortBy.value) {
        ...
        case 'category': {
          // Fallback to name as category is not easily accessible
          const nameA = getNeededItemData(a)?.name ?? '';
          const nameB = getNeededItemData(b)?.name ?? '';
          cmp = nameA.localeCompare(nameB);
          break;
  ...
  const groupedItems = computed((): GroupedNeededItem[] => {
    ...
    return Array.from(groups.values())
      ...
      .sort((a, b) => {
        let cmp = 0;
        switch (sortBy.value) {
          case 'priority': // Group priority? Fallback to total needed?
          case 'count':
            cmp = a.total - b.total;
            break;
          case 'name':

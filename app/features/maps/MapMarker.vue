<template>
  ...
  >
    <UIcon
      :name="
        tooltipVisible == true ? 'i-mdi-map-marker-radius' : 'i-mdi-map-marker'
      "
      class="w-5 h-5"
    />
  </div>
...
const relativeLocation = computed(() => {
  ...
  if (
    ...
    console.warn(
      "MapMarker: Invalid or missing map bounds for map:",
      props.map?.name
    );
  ...
  if (mapWidth === 0 || mapHeight === 0) {
    console.warn(
      "MapMarker: Map width or height is zero for map:",
      props.map?.name
    );
